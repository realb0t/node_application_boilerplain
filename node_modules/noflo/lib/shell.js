...
(function() {
  ...
  app.cmd("startserver :port", "Start graph HTTP server", function(req, res, next) {
    var server;
    if (!app.network) {
      app.styles.red("No graph is loaded");
      return res.prompt();
    ...
    return server.createServer(req.params.port, function(server) {
      app.server = server;
  ...
  app.cmd("stopserver", "Stop graph HTTP server", function(req, res, next) {
    if (!app.server) {
